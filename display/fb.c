...
static int FBDeviceInit(void)
{
	...
	g_fd = open(FB_DEVICE_NAME, O_RDWR); // /dev/fb0是linux系统下帧缓冲设备的设备文件 可直接改变内容在屏幕上画图
	if (0 > g_fd)
	{
	...
	g_pucFBMem = (unsigned char *)mmap(NULL , g_dwScreenSize, PROT_READ | PROT_WRITE, MAP_SHARED, g_fd, 0);//mmap 内存映射
	if (0 > g_pucFBMem)	
	{
	...
	g_tFBOpr.iLineWidth  = g_dwLineWidth;
...
static int FBShowPixel(int iX, int iY, unsigned int dwColor)
{
	...
	if ((iX >= g_tFBVar.xres) || (iY >= g_tFBVar.yres))//显示的位置不能超过X，Y最大坐标
	{
		DBG_PRINTF("out of region\n");
		return -1;
	}

	pucFB      = g_pucFBMem + g_dwLineWidth * iY + g_dwPixelWidth * iX; //得到开始显示的位置
...
int FBInit(void)
{
	return RegisterDispOpr(&g_tFBOpr); //将g_tFBOpr结构体放入链表中
}
